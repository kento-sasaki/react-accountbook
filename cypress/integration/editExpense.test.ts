describe('edit expense', () => {
  ...
  it('金額を編集', () => {
    ...
    cy.pause();
    cy.get('[data-test-id="expense-table"] > tbody > tr:first').contains('Edit').click();
    cy.get('[data-test-id="expense-table"] > tbody > tr:first > td:nth-child(2) > .ui > input')
      .clear()
      .type(`${amount}`);
    cy.get('[data-test-id="expense-table"] > tbody > tr:first > td:nth-child(4)')
      .contains('Save')
      .click();
    cy.pause();
  ...
  it('タグを編集', () => {
    ...
    cy.get('[data-test-id="expense-table"] > tbody > tr:first > td:nth-child(2) > .ui > input')
      ...
      .type(`${amount}`);
    cy.get(
      '[data-test-id="expense-table"] > tbody > tr:first > td:nth-child(3) i.dropdown.icon',
    ).click();
    cy.get(
      `[data-test-id="expense-table"] > tbody > tr:first > td:nth-child(3) .item > i.${tag.icon}.icon`,
    ).click();
    cy.get('[data-test-id="expense-table"] > tbody > tr:first > td:nth-child(4)')
      .contains('Save')
      .click();
